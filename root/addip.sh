...
cat $1 | awk '{print $1}' | awk '/^[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*$/' | sed 's/^/route /' | sed  's/$/\/32 via "'$2'";/' > $3
cat $1 | awk '{print $1}' | awk '/^[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*[\/]/' | sed 's/^/route /' | sed  's/$/ via "'$2'";/' >> $3

until ADDRS=$(dig +short google.com @localhost -p 53) && [ -n "$ADDRS" ] > /dev/null 2>&1; do sleep 5; done

echo "$(cat $1 | awk '{print $1}' | awk '!/^[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*/ && !/^#/')" | {
